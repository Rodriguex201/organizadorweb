...
<div class="mx-auto max-w-7xl px-4 py-8">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold">Cartera pendiente</h1>
            <p class="text-sm text-slate-600">Empresas con proformas pendientes en estados <code>Generada</code> y <code>Enviada</code>.</p>
        </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-lg bg-white p-4 shadow">
            <p class="text-xs uppercase text-slate-500">Empresas con deuda</p>
            <p class="mt-1 text-2xl font-bold text-slate-900">{{ number_format((int) $summary['empresas_con_deuda'], 0, ',', '.') }}</p>
        </div>
        <div class="rounded-lg bg-white p-4 shadow">
            <p class="text-xs uppercase text-slate-500">Total cartera</p>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <form method="GET" action="{{ route('proformas.cartera.index') }}" class="grid flex-1 gap-4 md:grid-cols-3 xl:grid-cols-5">
                ...
                <div class="flex items-end gap-2 xl:col-span-2">
                    ...
                    <a href="{{ route('proformas.cartera.index') }}" class="rounded bg-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-300">Limpiar</a>
                </div>
            </form>
    ...
    <div class="overflow-hidden rounded-lg bg-white shadow">
        <div class="overflow-x-auto rounded-lg">
            <table class="min-w-[1400px] text-sm">
                ...
                @empty
                    <tr>
                        <td colspan="12" class="px-4 py-8 text-center text-slate-500">No hay empresas con cartera pendiente para los filtros seleccionados.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
